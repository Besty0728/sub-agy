---
description: 检查 sub-agy 运行环境。触发词：诊断、doctor、环境、检查、健康、agy未装
argument-hint: ""
...
「您希望现在安装 sub-agy CLI 吗？」
- 选项 A: 「安装 sub-agy CLI（推荐）」
- 选项 B: 「暂不」
...
如果用户选择 B，告知用户可以稍后手动运行以上命令，或访问 sub-agy 官方文档了解安装方法。

### 无 uv 的情况

向用户提问一次（恰好一次）：

「您需要先安装 uv（Python 包与工具管理器），再安装 sub-agy。是否现在同时安装两者？」
- 选项 A: 「同时安装 uv 和 sub-agy」
- 选项 B: 「暂不」
